...
- `src/agents/` — Agent backends (opencode, direct-llm)
- `src/threads/manager.ts` — Thread lifecycle + concurrency
- `src/worktree/` — Git worktree CRUD + merge
- `src/compression/` — Result compression strategies
- `src/prompts/orchestrator.ts` — Swarm orchestrator system prompt
...
- `default_agent`: agent backend (default: opencode)
- `compression_strategy`: structured | diff-only | truncate
